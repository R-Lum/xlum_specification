...
The first level, `xlum` is a supernode to enable storage of luminescence data in arbitrary files following the XML scheme. As a side effect this also allows custom file endings different from `*.xlum`.
...
{...}
    ...
    <record recordType="bleach" sequenceStepNumber="2" sampleCondition="NA">
      <curve ...>
        {...}
      </curve>
    </record>
...
```{r, engine='bash', eval = FALSE}
{...}
<sequence position="1" name="SAR measuremt" fileName="Testsequence.seq" software="DeviceStudio, 2.37">
  <record ...>
    <curve ...>
      {...}
    </curve>
...
nature to ease the sequential of data storage during the measurement process.
  ...
  |parentID     |`token`         |yes                  |`"201007145551910"`   |unique identifying the parent node, the super node has the ID `0`, this is usually not needed but may help to store data |
