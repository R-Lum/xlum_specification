...
In 2011 Freiberg Instruments introduced a newly developed luminescence reader called lexsyg. 
Along with routine measurement data the system records a variety of addi- tional system information 
(e.g. LED power, heating ramp curves), which provide fully control over the experimental conditions. 
However, so far, in the luminescence dating community no appropriate data format exists to 
sufficiently store the obtained data.

With this documentation a new XML based file format (XSYG) is introduced to support the 
variety of needs of luminescence measurements. In contrast to the existing binary file 
format from Risø, which mainly focuses on low data volumes, the XSYG format, as an XML derivate, 
is easy to read and extendible. With this approach the various setups of the lexsyg measurement 
system is fully covered. As the format is human readable, the data can be easily checked with 
any kind of text reader, on any major operating system. This allows a platform independent operation.
The following documentation provides basic information on the file XSYG format and accounts 
for the needs of individual flexible data analysis, even with self written software applications.


# General description


First of all the XSYG format is an xml derivative. That means the based
data structure is a tree with nodes storing the data. The depth of the
tree is 4 levels. Any level has its unique denotation.

  level name   number of aliquots   number of measurements   number of raised data curves
  ------------ -------------------- ------------------------ ------------------------------
  sample       $\ast$               $\ast$                   $\ast$
  sequence     1                    $\ast$                   $\ast$
  record       1                    1                        $\ast$
...
the process(es) assigned to the node. An abstract example is shown in
...
comprehensible environment and abstract from there.
...
may in the same record as the heating (f.e. to document a TL).
  ...
  offset                                          number                  60                                     idle time till the component starts
...
measurement. Curves are stored in a record and used to describe it.
  ...
  recordType           Enumeration             TL                       legal values found in table x.x
  ...
  sampleCondition      Enumeration             dose                     legal values found in table x.x
