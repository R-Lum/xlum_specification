...
Two simple design principles underpin the format specification:
...
The levels and a short introduction is given in the table below. A detailed description can be found in the following sections. Nodes represent each level in the structure. **Only `<curve>` nodes contain luminescence data** all other nodes are parent nodes of `<curve>` 
...
may record only one or more curves.
...
by the detector are stored in the nodes.** Figure \@ref(fig:fig1) shows three types
of detectors, distinguished by their capacity to record spatially resolved information. 
...
knitr::include_graphics("img/Curve_Level_Storage.png", auto_pdf = TRUE, dpi = 200)
```
...
```{r, engine='bash', eval = FALSE}
{...}
  <curve stimulator="heating element" detector="thermo couple" 
    startDate="2021-02-14T225935.0Z" curveType="predefined" duaration="120" 
    offset="0" xValues="NA" yValues="NA" tValues="1,2,3,4,5" xLabel="NA" yLabel="NA"
    tLabel="time" xUnit="" yUnit="" tUnit="s">
        0,273,60,393,23
  </curve>
