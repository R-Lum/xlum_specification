...
With this documentation, a new XML based file format (`XLUM`) is introduced to support the variety of needs of luminescence measurements. In contrast to the existing binary file format from Risø, which mainly focuses on small data volumes, the XLUM format, as an XML derivative, is easy to read and extendible. The setup accounts for various configurations of the *lexsyg* measurement system but can be also adapted for other measurements systems. As the format is human readable, the data can be easily checked with any kind of text reader, on any major operating system. This allows a platform independent operation. The following documentation provides basic information on the file XLUM format and accounts for the needs of individual flexible data analysis, even with self-written software applications.


# General description

The XLUM-format is an XML derivative, i.e. the base
