...
may consist of one or many curves.
 ...
 (temperature sensor), (b) time against photon counts recorded by a photomultiplier tube, (c) time against a predefined heating ramp.
...
*Please note that the XSD document does not specify binary encoding considers all values in `<curve/>` strings (which can be encoded).*
...
physical information a stored in the `<curve>` node.
  ...
  |`"BSL"`               | blue optically stimulated luminescence                    |
...
valid entries for  `sampleConditions` are:
  ...
  |`"Nat.(Bleach)"`       |  Naturally depleted luminescence |
  |`"Nat.+Dose(Bleach)"`  |  Naturally depleted luminescence with dose |
...
A few attributes are available on all node levels except the root node `<xlum>`. There are mainly of technical 
nature to ease the sequential of data storage during the measurement process.
  ...
  |parentID     |`token`         |yes                  |`"201007145551910"`   |unique identifying the parent node, the root node has the ID `0`, this is usually not needed but may help to store data |
...
to understand the data and the undertaken processing on the hardware level better. All attributes
are optional.
...
## BIN/BINX to XLUM metadata argument matching

The BIN/BINX files consist of a metadata header block and a data block. Some of 
the metadata serve the sole purpose of describing the structure of the data block 
others return the specific status of equipment, but are non-relevant for the data analysis. 

Most information is covered by the XLUM format by design and hence find their 
expression implicitly. The following argument list is a master list containing
**all** possible arguments in BIN/BINX files. Not all BIN/BINX
...
|04|`RECTYPE`         |NONE               |non-relevant internal metadata       |
...
|10|`CRUVENO`         |implicit           |through structure                    |
